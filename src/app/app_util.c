...
///leo add
#if BLE_QPP_CLIENT
        case QPP_SVC_PRIVATE_UUID:
            enabled = app_qppc_env->enabled;
            break;
#endif
///leo end
#if BLE_PAS_CLIENT
...
///leo add
#if BLE_QPP_CLIENT
        if (conhdl == app_qppc_env->conhdl)
        {
            app_qppc_env->enabled = false;
        }
#endif
///leo end
#if BLE_PAS_CLIENT
...
///leo add
#if BLE_QPP_SERVER
	app_qpps_create_db(QPPS_VAL_CHAR_NUM);
#endif

#ifdef TASK_OTAS
    app_otas_create_db(OTAS_RX_CHAR_NUM);
#endif
///leo end
#if (BLE_AN_SERVER)
...
///leo add
    ...
    if (enabled == true)
    {
        ...
        app_qpps_enable_req(conhdl, PERM_RIGHT_ENABLE, PRF_CON_NORMAL, app_qpps_env->features);
        app_qpps_env->enabled = true;
		
		app_otas_enable_req(conhdl, PERM_RIGHT_ENABLE, PRF_CON_NORMAL);
...
///leo end
    ...
    if (enabled == true)
    {
        const int8_t pa_gain2dbm_table[] = {-20, -18, -16, -15, -14, -12, -10, -11, -8, -6, -4, -3, -2, 0, 2, 4};
#if !QN_WORK_MODE
        const int8_t pa_gain = rf_tx_power_level_get();
#else
        const int8_t pa_gain = 0;
#endif
...
uint8_t app_set_adv_data(uint16_t disc_mode)
{
    ...
    app_env.adv_data[0] = GAP_AD_TYPE_MORE_16_BIT_UUID;
    app_env.adv_data[1] = 0x01;
...
#if (BLE_QPP_SERVER || BLE_AN_SERVER || BLE_CSC_SENSOR || BLE_PAS_SERVER || BLE_HT_THERMOM || BLE_RSC_SENSOR || BLE_BP_SENSOR | BLE_HR_SENSOR | BLE_GL_SENSOR | BLE_TIP_SERVER | BLE_SP_SERVER | BLE_HID_DEVICE | BLE_PROX_REPORTER | BLE_FINDME_TARGET | BLE_DIS_SERVER | BLE_BATT_SERVER)
    uint8_t remain_len = 23-2;
#endif
    bool complete = true;
    len = 2;
///leo add
...
//	if (len <= remain_len)
//	{
//		if (srv_flag & BLE_QPP_SERVER_BIT)
//		{
...
//         app_env.scanrsp_data[len+1] = (uint8_t)(OTAS_SVC_PRIVATE_UUID>>8);
        app_env.scanrsp_data[len+0] = (uint8_t)(0xcc07&0x00FF);
        app_env.scanrsp_data[len+1] = (uint8_t)(0xcc07>>8);
        len += 2;
	}
	else
	{
