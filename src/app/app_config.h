...
#define WORK_MODE_HCI               2
    ...
    #define QN_EAPI                 0
		...
		#define QN_PT_BOTTON								1		
#else
		#define QN_PT_BOTTON								0
#endif
...
/// Wakeup external mcu
#if (defined(CFG_EACI_GPIO_WAKEUP_EX_MCU))
...
#if (defined(CFG_WM_NP) || defined(CFG_WM_HCI) || defined(CFG_EACI) || defined(CFG_TEST_CTRL_PIN) || defined(CFG_PT))
    #define QN_HCI_UART                 CFG_HCI_UART
    #define QN_HCI_UART_RD              uart_read
    #define QN_HCI_UART_WR              uart_write
#else
    #define QN_HCI_UART                 NULL
    #define QN_HCI_UART_RD              NULL
...
/// Size of the heap
    ...
    /// some heap must be reserved for attribute database
        ...
        #define BLE_DB_SIZE         (GAP_DB_SIZE+GATT_DB_SIZE+HTPT_DB_SIZE+PXPR_DB_SIZE+FMPT_DB_SIZE+DISS_DB_SIZE+ BLPS_DB_SIZE+TIPS_DB_SIZE+HRPS_DB_SIZE+HOGPD_DB_SIZE+SCPPS_DB_SIZE+BASS_DB_SIZE+GLPS_DB_SIZE+RSCPS_DB_SIZE+PASPS_DB_SIZE+CSCPS_DB_SIZE+ANPS_DB_SIZE+QPPS_DB_SIZE+OTAS_DB_SIZE)
    #else
        #define BLE_DB_SIZE         (0)
    #endif /* (BLE_ATTS || BLE_ATTC) */

    #define BLE_HEAP_SIZE           (BLE_DB_SIZE + 1024 + 256 * BLE_CONNECTION_MAX)
#else
    #define BLE_HEAP_SIZE           (256)
