...
/// Procedure Timeout Timer Value (*10ms)
#define RSCPC_PROC_TIMEOUT_TIMER_VAL         (3000)
                                  ...
                                  ke_task_id_t const src_id)
{
    ...
    if (rscpc_env != NULL)
    {
        ...
        if (param->conhdl == rscpc_env->con_info.conhdl)
        {
                                             ...
                                             ke_task_id_t const src_id)
{
    ...
    if (rscpc_env != NULL)
    {
        ...
        if (rscpc_env->nb_svc == 0)
        {
            ...
            rscpc_env->rscs.svc.shdl = param->list[0].start_hdl;
            rscpc_env->rscs.svc.ehdl = param->list[0].end_hdl;
            rscpc_env->nb_svc++;
        }
    }
    // else drop the message
            ...
            else
            {
                ...
                if (disc_over)
                {
                    ...
                    if (status == PRF_ERR_OK)
                    {
                        ...
                        ind->conhdl = rscpc_env->con_info.conhdl;
    ...
    if (rscpc_env != NULL)
    {
        ...
        if (param->status == GATT_NO_ERROR)
        {
            ...
            switch (((struct rscpc_read_cmd *)rscpc_env->operation)->read_code)
            {
                ...
                case (RSCPC_RD_SENSOR_LOC):
                {
                    ind->value.sensor_loc = (param->data.data[0]< RSCP_LOC_MAX) ? param->data.data[0] : RSCP_LOC_OTHER;
                } break;
        ...
        switch (operation)
        {
            ...
            case (RSCPC_CTNL_PT_CFG_WR_OP_CODE):
            {
                if (param->status == PRF_ERR_OK)
                {
                    ...
                    ke_timer_set(RSCPC_TIMEOUT_TIMER_IND, dest_id, RSCPC_PROC_TIMEOUT_TIMER_VAL);
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    if (rscpc_env != NULL)
    {
        ...
        // Instantaneous Speed
        ind->value.rsc_meas.inst_speed = co_read16p(&param->value[1]);
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    PRF_CLIENT_DISABLE_IND_SEND(rscpc_envs, dest_id, RSCPC);
...
ke_state_t rscpc_state[RSCPC_IDX_MAX];

// Register RSCPC TASK into kernel
void task_rscpc_desc_register(void)
{
    struct ke_task_desc task_rscpc_desc;

    task_rscpc_desc.state_handler = rscpc_state_handler;
    task_rscpc_desc.default_handler = &rscpc_default_handler;
    task_rscpc_desc.state = rscpc_state;
    task_rscpc_desc.state_max = RSCPC_STATE_MAX;
    task_rscpc_desc.idx_max = RSCPC_IDX_MAX;

    task_desc_register(TASK_RSCPC, task_rscpc_desc);
}
