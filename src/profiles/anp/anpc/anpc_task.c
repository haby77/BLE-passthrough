                                  ...
                                  ke_task_id_t const src_id)
{
    ...
    if (anpc_env != NULL)
    {
        ...
        if (param->conhdl == anpc_env->con_info.conhdl)
        {
            ...
            else    // state = ANPC_CONNECTED
            {
                ...
                if (status == PRF_ERR_OK)
                {
                    ...
                    if (handle != ATT_INVALID_SEARCH_HANDLE)
                    {
                        ...
                        prf_gatt_write(&(anpc_env->con_info), handle, (uint8_t *)&param->value, length, GATT_WRITE_CHAR);
                                             ...
                                             ke_task_id_t const src_id)
{
    ...
    if (anpc_env != NULL)
    {
        ...
        if (anpc_env->nb_svc == 0)
        {
            ...
            anpc_env->ans.svc.shdl = param->list[0].start_hdl;
            anpc_env->ans.svc.ehdl = param->list[0].end_hdl;
            anpc_env->nb_svc++;
        }
    }
            ...
            else
            {
                ...
                else if (anpc_env->last_req == ANPC_CHAR_UNREAD_ALERT_STATUS)
                {
                    ...
                    if (status == PRF_ERR_OK)
                    {
                        ...
                        ind->conhdl = anpc_env->con_info.conhdl;
        ...
        switch (((struct anpc_cmd *)anpc_env->operation)->operation)
        {
            ...
            case (ANPC_ENABLE_RD_NEW_ALERT_OP_CODE):
            {
                if (param->status == GATT_NO_ERROR)
                {
                    ...
                    ind->value.supp_cat.cat_id_mask_0 = param->data.data[0];
                    // If cat_id_mask_1 not present, shall be considered as 0
                    ind->value.supp_cat.cat_id_mask_1 = (param->data.len > 1) ? param->data.data[1] : 0x00;
            ...
            case (ANPC_ENABLE_RD_UNREAD_ALERT_OP_CODE):
            {
                if (param->status == GATT_NO_ERROR)
                {
                    ...
                    ind->value.supp_cat.cat_id_mask_0 = param->data.data[0];
                    // If cat_id_mask_1 not present, shall be considered as 0
                    ind->value.supp_cat.cat_id_mask_1 = (param->data.len > 1) ? param->data.data[1] : 0;
                    ...
                    else
                    {
                        ...
                        anpc_found_next_alert_cat(anpc_env, ANP_NEW_ALERT,
                                                  ((struct anpc_enable_cmd *)anpc_env->operation)->new_alert_enable);

                        // Enable sending of notifications for the found category ID
                        anpc_write_alert_ntf_ctnl_pt(anpc_env, CMD_ID_EN_NEW_IN_ALERT_NTF, anpc_env->last_req);
                    }
                }
                else
                {
            ...
            case (ANPC_READ_OP_CODE):
            {
                if (param->status == GATT_NO_ERROR)
                {
                    ...
                    ind->value.ntf_cfg = co_read16p(&param->data.data[0]);
    ...
    if (anpc_env != NULL)
    {
        ...
        switch (operation)
        {
            ...
            case (ANPC_ENABLE_WR_NEW_ALERT_OP_CODE):
            {
                ...
                if (anpc_found_next_alert_cat(anpc_env, ANP_NEW_ALERT,
                                              ((struct anpc_enable_cmd *)anpc_env->operation)->new_alert_enable))
                {
                    // Enable sending of notifications for the found category ID
                    anpc_write_alert_ntf_ctnl_pt(anpc_env, CMD_ID_EN_NEW_IN_ALERT_NTF, anpc_env->last_req);
            ...
            case (ANPC_ENABLE_WR_NTF_NEW_ALERT_OP_CODE):
            {
                ...
                // Look for the next Unread Alert Category to enable
                anpc_found_next_alert_cat(anpc_env, ANP_UNREAD_ALERT,
                                          ((struct anpc_enable_cmd *)anpc_env->operation)->unread_alert_enable);

                // Write the Alert Notification Control Point Characteristic value
                anpc_write_alert_ntf_ctnl_pt(anpc_env, CMD_ID_EN_UNREAD_CAT_STATUS_NTF, anpc_env->last_req);
            } break;

            case (ANPC_ENABLE_WR_UNREAD_ALERT_OP_CODE):
            {
                // Look for the next category to enable
                if (anpc_found_next_alert_cat(anpc_env, ANP_UNREAD_ALERT,
                                              ((struct anpc_enable_cmd *)anpc_env->operation)->unread_alert_enable))
                {
                    // Enable sending of notifications for the found category ID
                    anpc_write_alert_ntf_ctnl_pt(anpc_env, CMD_ID_EN_UNREAD_CAT_STATUS_NTF, anpc_env->last_req);
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    if (anpc_env != NULL)
    {
        ...
        if (param->value[0] < CAT_ID_NB)
        {
            ...
            if (param->charhdl == anpc_env->ans.chars[ANPC_CHAR_NEW_ALERT].val_hdl)
            {
                // Text String information Length
                uint8_t length = param->size - 2;
                ...
                new_alert_ind->att_code = ANPC_CHAR_NEW_ALERT;
                new_alert_ind->conhdl = param->conhdl;
            ...
            else if (param->charhdl == anpc_env->ans.chars[ANPC_CHAR_UNREAD_ALERT_STATUS].val_hdl)
            {
                // Send an indication to the application
                ...
                unrd_alert_ind->att_code = ANPC_CHAR_UNREAD_ALERT_STATUS;
                unrd_alert_ind->conhdl = param->conhdl;
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    PRF_CLIENT_DISABLE_IND_SEND(anpc_envs, dest_id, ANPC);
...
ke_state_t anpc_state[ANPC_IDX_MAX];

// Registet ANPC task into kernel
void task_anpc_desc_register(void)
{
    struct ke_task_desc task_anpc_desc;
    
    task_anpc_desc.state_handler = anpc_state_handler;
    task_anpc_desc.default_handler= &anpc_default_handler;
    task_anpc_desc.state = anpc_state;
    task_anpc_desc.state_max = ANPC_STATE_MAX;
    task_anpc_desc.idx_max = ANPC_IDX_MAX;
    
    task_desc_register(TASK_ANPC, task_anpc_desc);
}
#endif //(BLE_AN_CLIENT)
