...
#define CSCPC_PROC_TIMEOUT_TIMER_VAL         (3000)
...
ke_state_t cscpc_state[CSCPC_IDX_MAX];

// Register CSCPC TASK into kernel
void task_cscpc_desc_register(void)
{
    struct ke_task_desc task_cscpc_desc;

    task_cscpc_desc.state_handler = cscpc_state_handler;
    task_cscpc_desc.default_handler = &cscpc_default_handler;
    task_cscpc_desc.state = cscpc_state;
    task_cscpc_desc.state_max = CSCPC_STATE_MAX;
    task_cscpc_desc.idx_max = CSCPC_IDX_MAX;

    task_desc_register(TASK_CSCPC, task_cscpc_desc);
}
